...
teamsManager.minReAlertSec = 3 -- to prevent a bunch at once
deleteDestroyed = false -- For possible RAM concerns. Though, in the few small tests I tried it didn't seem to save more RAM

alertAllTaken = true -- NOT TESTED YET
alertAllTakenRules = {anyTaken = {["alertAllTaken"] = {sharedAlerts=true, reAlertSec=3, mark="Unit Taken"}}} -- KEEP "sharedAlerts=true", else this can SPAM
alertAllGiven = true
alertAllGivenRules = {anyGiven = {["alertAllGiven"] = {sharedAlerts=true, reAlertSec=3, mark="Unit Given"}}} -- KEEP "sharedAlerts=true", else this can SPAM
teamsManager.logEvents = true
debug = false
-- local logEventsTbl = {}
...
-- function teamsManager:loadCustomGroups()
--   if debug then debugger("loadCustomGroups 1.") end
--   if myCustomGroups ~= nil and type(myCustomGroups) ~= "table" then debugger("loadCustomGroups ERROR. Not Table. myCustomGroups="..tostring(myCustomGroups)); return false end
--   if allyCustomGroups ~= nil and type(allyCustomGroups) ~= "table" then debugger("loadCustomGroups ERROR. Not Table. allyCustomGroups="..tostring(allyCustomGroups)); return false end
--   if enemyCustomGroups ~= nil and type(enemyCustomGroups) ~= "table" then debugger("loadCustomGroups ERROR. Not Table. enemyCustomGroups="..tostring(enemyCustomGroups)); return false end
--   if spectatorCustomGroups ~= nil and type(spectatorCustomGroups) ~= "table" then debugger("loadCustomGroups ERROR. Not Table. spectatorCustomGroups="..tostring(spectatorCustomGroups)); return false end
--   local countTR = 0
--   if debug then debugger("loadCustomGroups 2. Assigning types with myCustomGroups=" ..tableToString(myCustomGroups) .. ", =") end
--   local tmpTypesRules = {[1]=trackMyTypesRules,[2]=trackAllyTypesRules,[3]=trackEnemyTypesRules,[4]=trackSpectatorTypesRules}
--   local tmpCustomGroups = {[1]=myCustomGroups,[2]=allyCustomGroups,[3]=enemyCustomGroups,[4]=spectatorCustomGroups}
...
--     end
--   end
--   if debug then debugger("loadCustomGroups 6. Assigning types with typesRules[grpName]=" ..tableToString(trackMyTypesRules["groupName1"])) end
--   return true
-- end
--====================================================
...
--   funcCounts.numArmies = funcCounts.numArmies + 1
...
-- 	funcCounts.numGetArmy = funcCounts.numGetArmy + 1
...
-- 	funcCounts.numIfInitialized = funcCounts.numIfInitialized + 1
...
-- 	funcCounts.numIsAllied = funcCounts.numIsAllied + 1
...
-- 	funcCounts.numMoveUnit = funcCounts.numMoveUnit + 1
...
-- 	funcCounts.numValidIDs = funcCounts.numValidIDs + 1
...
-- 	funcCounts.numAlert = funcCounts.numAlert + 1
...
-- 	funcCounts.numAddAlert = funcCounts.numAddAlert + 1
...
--       end
--     end
--     teamsManager:addToEventTbl(alertVarsTbl,funcCounts.numAddAlert)
--   end
...
-- 	funcCounts.numGetNotifyVars = funcCounts.numGetNotifyVars + 1
...
-- 	funcCounts.numGetNextAlert = funcCounts.numGetNextAlert + 1
...
-- 	funcCounts.numGetQueued = funcCounts.numGetQueued + 1
...
-- 	funcCounts.numValidRules = funcCounts.numValidRules + 1
...
-- 	funcCounts.numCreateUnit = funcCounts.numCreateUnit + 1
...
--   funcCounts.numGetUnit = funcCounts.numGetUnit + 1
...
--   funcCounts.numGetOrCreate = funcCounts.numGetOrCreate + 1
...
--   funcCounts.numGetRulesForEvent = funcCounts.numGetRulesForEvent + 1
...
--   funcCounts.numCanAlert = funcCounts.numCanAlert + 1
...
--   funcCounts.numHasEventRules = funcCounts.numHasEventRules + 1
...
--   funcCounts.numSetIdle = funcCounts.numSetIdle + 1
...
--   funcCounts.numSetNotIdle = funcCounts.numSetNotIdle + 1
...
--   funcCounts.numGetIdle = funcCounts.numGetIdle + 1
...
--   funcCounts.numSetLost = funcCounts.numSetLost + 1
...
--   funcCounts.numSetTypes = funcCounts.numSetTypes + 1
...
--   funcCounts.numHasRules = funcCounts.numHasRules + 1
...
--   funcCounts.numGetCoords = funcCounts.numGetCoords + 1
...
--   funcCounts.numGetHealth = funcCounts.numGetHealth + 1
...
--       relevantMyUnitDefsRules[defID][key] = playerRules[key]
--     end
...
--       relevantAllyUnitDefsRules[defID][key] = allyRules[key]
--     end
...
--       relevantEnemyUnitDefsRules[defID][key] = enemyRules[key]
--     end
--   else
...
--       relevantSpectatorUnitDefsRules[defID][key] = spectatorRules[key]
--     end
--   end
-- end

-- function teamsManager:makeRelTeamDefsRules(playerRules, allyRules, enemyRules, spectatorRules) -- This should ensure that types are only added to armyManager if there are events defined, and validate events/rules
...
--           if string.find(unitDef.translatedTooltip:lower(), string.lower("rapid")) or string.find(unitDef.translatedTooltip:lower(), string.lower("plasma")) then
...
--         end
--       end
--       for i,cstmGroupRules in ipairs({myCustomGroups,allyCustomGroups,enemyCustomGroups,spectatorCustomGroups}) do
...
--               teamsManager:addToRelTeamDefsRules(defID, grpName)
--             end
--           end
--         end
--       end
...
-- function teamsManager:Initialize(playerEvents, allyEvents, enemyEvents, spectatorEvents)
--   if not teamsManager:loadCustomGroups() or not teamsManager:makeRelTeamDefsRules(playerEvents, allyEvents, enemyEvents, spectatorEvents) then
--     return false
--   end
...
function widget:Shutdown()
  ...
  if teamsManager.logEvents then
    local gameID = Game.gameID and Game.gameID or Spring.GetGameRulesParam("GameID")
    saveTable("myData", teamsManager.logEventsTbl, ("ea"..gameID..".lua"))
...
--
-- Saving a table to file
-- Convert the table to a string, write it to a new lua file, have that file also return the table, then  `local my_table = VFS.Include(my_file_path)`
-- Something like that is how I would try to do it.
-- Save function
...
local function loadTable(filename)
    ...
    if file then
        ...
        file:close()
        debugger("loadTable1. content="..tostring(content))
        local func, message = loadstring(content)
        if func then
            return func()
        else
            debugger("Error: Could not load table from file. Message="..tostring(message))
        end
    else
        debugger("Error: Could not open file for reading.")
    end
    return nil
end
...
--     ["version"] = 5.4,
--     ["isActive"] = true,
--     -- ["features"] = {"serialization", "deserialization", "booleans"},
--     ["isEnabled"] = false
-- }

-- You'd need a robust table.dump function for general use.
-- Here's a simplified version for demonstration:


-- Save the table
-- saveTable("myData", myData, "data.lua")

-- Load the table
-- local loadedData = loadTable("data.lua")

-- -- Verify loaded data
-- if loadedData then
--     debugger("Loaded table:")
--     for k, v in pairs(loadedData) do
--         debugger(k, "=", v)
--     end
--     debugger("Is isActive boolean?", type(loadedData.isActive) == "boolean")
--     debugger("Is isEnabled boolean?", type(loadedData.isEnabled) == "boolean")
-- end
...
-- Lua code as a string to create a nested table
-- local table_string = [[
--     return {
--         name = "My_Object",
--         properties = {
...
-- Load and execute the string as Lua code
-- local func = loadstring(table_string) -- table_string

-- -- Check if loading was successful
-- if func then
--     -- Call the function to get the table
--     local my_nested_table = func()

--     -- Access values from the nested table
--     debugger("Name: " .. my_nested_table.name)
--     debugger("Color: " .. my_nested_table.properties.color)
--     debugger("Width: " .. my_nested_table.properties.dimensions.width)
--     debugger("First item: " .. my_nested_table.items[1])
--     debugger("aBlurb: " .. tostring(my_nested_table.aBlurb))

-- else
--     debugger("Error loading string: " .. tostring(func))
-- end


-- eCount = 0
-- function doTesting()
--   -- -- tmsMgr:getArmyManager(teamID)
--   -- local com = tmsMgr:getOrCreateUnit(19913,282,0) -- defID 282
--   -- local vars = tmsMgr:getEventRulesNotifyVars(com,com:getTypesRulesForEvent("idle", true, true) )
--   -- -- debugger("tableToString: " .. tableToString(vars)) -- works
--   -- vars["gameSec"] = Spring.GetGameSeconds()
--   -- vars["unit"] = {unitID=com.ID,defID=com.defID, teamID=com.parent.teamID}
--   -- com:getCoords()
--   -- vars["coords"] = {x=com.coords["x"], y=com.coords["y"], z=com.coords["z"]}

--   -- aConcat = "e"..tostring(eCount)
--   -- vars = {[aConcat] = vars}
--   -- saveTable("myData", vars, "data.lua")
--   -- eCount = eCount + 1
--   -- -- alertPointer(vars.coords.x, vars.coords.y, vars.coords.z, vars.mark, true)
--   -- local my_table = VFS.Include("data.lua") -- works
--   -- -- debugger("String: " .. tableToString(my_table)) -- works
...
-- getCurrentMiniMapRotationOption = VFS.Include("luaui/Include/minimap_utils.lua").getCurrentMiniMapRotationOption